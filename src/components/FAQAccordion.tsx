...
export default function FAQAccordion({ faqs }: FAQAccordionProps) {
  ...
  // Build category tabs dynamically from actual FAQ data
  const seenSlugs = new Set<string>()
  const dynamicCategories: { value: string; labelFr: string; labelEn: string }[] = []
  for (const item of items) {
    if (item.category && typeof item.category === 'string' && !seenSlugs.has(item.category)) {
      seenSlugs.add(item.category)
      dynamicCategories.push({
        value: item.category,
        labelFr: item.categoryTitle || item.category,
        labelEn: item.categoryTitleEn || item.categoryTitle || item.category,
      })
    }
  }

  // Show category tabs only when there are 2+ distinct categories
  const showCategories = dynamicCategories.length >= 2
  const categories = showCategories
    ? [{ value: 'all', labelFr: 'Toutes', labelEn: 'All' }, ...dynamicCategories]
    : []
